...
public class ExpandableTextView extends AppCompatTextView {
    ...
    private android.view.animation.Animation mOpenAnim, mCloseAnim;
    ...
    private void initialize() {
        mOpenSuffixColor = mCloseSuffixColor = android.graphics.Color.parseColor("#F23030");
    ...
    public void setOriginalText(CharSequence originalText) {
        ...
        if (maxLines != -1) {
            android.text.Layout layout = createStaticLayout(tempText);
            ...
            if (mExpandable) {
                ...
                //循环判断，收起内容添加展开后缀后的内容
                android.text.Layout tempLayout = createStaticLayout(tempText2);
...
//                    }
    ...
    private void open() {
        if (hasAnimation) {
            android.text.Layout layout = createStaticLayout(mOpenSpannableStr);
    ...
    private void executeOpenAnim() {
        ...
        if (mOpenAnim == null) {
            ...
            mOpenAnim.setAnimationListener(new android.view.animation.Animation.AnimationListener() {
                @Override
    ...
    private void executeCloseAnim() {
        ...
        if (mCloseAnim == null) {
            ...
            mCloseAnim.setAnimationListener(new android.view.animation.Animation.AnimationListener() {
                @Override
                public void onAnimationStart(android.view.animation.Animation animation) {
     ...
     * @return
     */
private android.text.Layout createStaticLayout(SpannableStringBuilder spannable) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M){
        ...
        return new StaticLayout(spannable, getPaint(), contentWidth, android.text.Layout.Alignment.ALIGN_NORMAL,
    ...
    private void updateCloseSuffixSpan() {
        ...
        if (mCloseInNewLine) {
            AlignmentSpan alignmentSpan = new AlignmentSpan.Standard(android.text.Layout.Alignment.ALIGN_OPPOSITE);
    ...
    class ExpandCollapseAnimation extends android.view.animation.Animation {
        private final android.view.View mTargetView;//动画执行view
        private final int mStartHeight;//动画执行的开始高度
        private final int mEndHeight;//动画结束后的高度

        ExpandCollapseAnimation(android.view.View target, int startHeight, int endHeight) {
            mTargetView = target;
